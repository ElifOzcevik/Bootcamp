    ...
    switch(subscriber.id){
        case "accountsList":
            list.forEach((element)=>{
                ...
                newListItem.setAttribute("class", "list-group-item d-flex justify-content-between align-items-center");
        ...
        case "historyList":
            list.forEach((element)=>{
                ...
                newListItem.innerHTML = `<div class="ms-2 me-auto">${element.time.toLocaleString()} - ${element.sender} sent ${element.receiver} ${element.amount} &#8378</div><button class="btn btn-danger btn-sm" onclick="cancelTransaction(event)">Cancel</button>`;
                subscriber.appendChild(newListItem);
            });
            break;
        case "productList":
            list.forEach((element)=>{
                const newListItem = document.createElement("li");
                newListItem.setAttribute("class", "list-group-item d-flex align-items-center justify-content-between");
...
function Option(list, subscriber) {
    list.forEach((element)=>{
        ...
        if (state.cart.currentUser === element.id) newOption.setAttribute("selected", true);
    ...
    optionList.forEach(function(subscriber) {
        switch(subscriber.id){
            ...
            case "receiverFilter":
                ...
                break;
            case "cartUser":
                if (state.cart.currentUser === "") subscriber.innerHTML = `<option value="" disabled selected hidden>User</option>`;
                else subscriber.innerHTML = ``;
                break;
            default:
                break;
        }
...
function filterList() {
    ...
    if (filterList.sender === "" && filterList.receiver === "") return historyList;
    else if (filterList.receiver === "") // Alıcı filtresi boşsa, gönderen filtresine göre filtreleme
    return historyList.filter((history)=>history.senderId === filterList.sender);
    else if (filterList.sender === "") // Gönderici filtresi boş ise alıcı filtresine göre filtreleme
    return historyList.filter((history)=>history.receiverId === filterList.receiver);
    else if (filterList.sender === filterList.receiver) // Gönderici ve alıcı filtresi aynı ise, o hesap kimliğine sahip tüm işlemleri getir
    return historyList.filter((history)=>{
        return history.senderId === filterList.sender || history.receiverId === filterList.receiver;
    });
    else // Gönderici ve alıcı filtresi varsa göndericinin tüm işlemlerini alıcıya getirin
    return historyList.filter((history)=>{
        return history.senderId === filterList.sender && history.receiverId === filterList.receiver;
    });
}
...
function addAccount() {
    ...
    if (!accountName.value || !accountBalance.value) return alert("Invalid account!");
...
function removeAccount(event) {
    ...
    if (state.cart.currentUser === accountId) setState("cart", {
        currentUser: "",
        products: [],
        total: 0
    });
...
function sendMoney() {
    ...
    const receiverId = receiverOptions.options[receiverOptions.selectedIndex].value; // Option değerinden alıcının kimliğini alma
    ...
    if (senderId === receiverId || !amount) return alert("Invalid transaction!");
...
function addHistory(senderId, receiverId, amount) {
    ...
    const receiver = state.accountList.find((account)=>account.id === receiverId); // Alıcı hesabı nesnesini kimliğine göre alma
...
function deleteHistory(historyId) {
    const historyList = state.historyList.filter((history)=>history.id !== historyId); //id'ye göre işlem geçmişi silme
...
function cancelTransaction(event) {
    ...
    const sender = state.accountList.find((account)=>account.id === history.senderId); //İd'ye göre stateden gönderici nesnesini alma
    const receiver = state.accountList.find((account)=>account.id === history.receiverId); //İd'ye göre stateden alıcı nesnesini alma
    // Hesap silindiyse uyarı mesajı verme
    if (!sender || !receiver) return alert("One of the accounts is deleted. You cannot cancel this transaction!");
    else {
...
function changeBalance(accountId, amount) {
    ...
    const accountIndex = accountList.findIndex((account)=>account.id === accountId); // Hesabın indeksini alma   //BURADA İF İLE KONTROL KOY BALANCE AMOUNTTAN KÜÇÜKSE UYARI MESAJI VERSİN
...
function changeStock(cartProductList, sign) {
    ...
    if (sign === "+") cartProductList.forEach((cartProduct, index)=>{
    ...
    else if (sign === "-") cartProductList.forEach((cartProduct, index)=>{
        const productIndex = productList.findIndex((product)=>product.id === cartProduct.id);
        productList[productIndex] = {
            ...productList[productIndex],
...
function addProduct() {
    ...
    if (productName && productPrice && productQuantity) {
        ...
        renderProducts();
    } else alert("Invalid product input!");
}
// Ürün render metodu
...
function addToCart(event) {
    if (!state.cart.currentUser) alert("No user selected for cart!");
    else {
        ...
        if (productIndex !== -1) cartProducts[productIndex] = {
